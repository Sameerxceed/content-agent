...
<?php elseif ($action === '_old_view_disabled' && isset($_GET['id'])):
    ...
    if (!$site):
        ...
        $stmt->execute([$site['id']]);
        $post_counts = [];
        foreach ($stmt->fetchAll() as $r) $post_counts[$r['status']] = $r['cnt'];
...
?>
    <div class="flex justify-between items-center mb-4">
        <div>
            <span class="text-muted text-sm"><?= e($site['domain']) ?></span>
            <?php if ($site['platform']): ?>
                <span class="badge badge-info"><?= e($site['platform']) ?></span>
            <?php endif; ?>
        </div>
        <div class="flex gap-2">
            <a href="<?= url('/dashboard/sites.php?action=edit&id=' . $site['id']) ?>" class="btn btn-outline btn-sm">Edit</a>
            <a href="<?= url('/dashboard/seo-audit.php?site=' . $site['id']) ?>" class="btn btn-outline btn-sm">SEO Audit</a>
            <a href="<?= url('/dashboard/keywords.php?site=' . $site['id']) ?>" class="btn btn-outline btn-sm">Keywords</a>
        </div>
    </div>

    <div class="stats-grid">
        <a href="<?= url('/dashboard/seo-audit.php?site=' . $site['id']) ?>" class="stat-card" style="text-decoration:none;color:inherit;cursor:pointer;">
            <div class="stat-label">SEO Score</div>
            <div class="stat-value"><?= $audit ? $audit['score'] . '/100' : '—' ?></div>
            <div class="stat-sub"><?= $audit ? format_date($audit['run_at'], 'd M') : 'Not audited' ?></div>
        </a>
        <a href="<?= url('/dashboard/posts.php?site=' . $site['id'] . '&status=published') ?>" class="stat-card" style="text-decoration:none;color:inherit;cursor:pointer;">
            <div class="stat-label">Published</div>
            <div class="stat-value"><?= $post_counts['published'] ?? 0 ?></div>
        </a>
        <a href="<?= url('/dashboard/posts.php?site=' . $site['id'] . '&status=draft') ?>" class="stat-card" style="text-decoration:none;color:inherit;cursor:pointer;">
            <div class="stat-label">Drafts</div>
            <div class="stat-value"><?= $post_counts['draft'] ?? 0 ?></div>
        </a>
        <a href="<?= url('/dashboard/keywords.php?site=' . $site['id']) ?>" class="stat-card" style="text-decoration:none;color:inherit;cursor:pointer;">
    ...
    <div class="grid-2">
        <div class="card">
            ...
            <table>
                <tr><td class="text-muted" style="width:120px">Tone</td><td><?= e($site['brand_tone'] ?? 'Not analyzed') ?></td></tr>
                <tr><td class="text-muted">Colors</td><td>
                    <?php foreach (json_decode($site['brand_colors'] ?? '[]', true) ?: [] as $color): ?>
                        <span style="display:inline-block;width:20px;height:20px;background:<?= e($color) ?>;border-radius:3px;margin-right:4px;vertical-align:middle;border:1px solid #ddd"></span>
                    <?php endforeach; ?>
                    <?php if (empty(json_decode($site['brand_colors'] ?? '[]', true))): ?>—<?php endif; ?>
                </td></tr>
                <tr><td class="text-muted">Fonts</td><td><?= e(implode(', ', json_decode($site['brand_fonts'] ?? '[]', true) ?: [])) ?: '—' ?></td></tr>
                <tr><td class="text-muted">Topics</td><td><?= e(implode(', ', json_decode($site['topics'] ?? '[]', true) ?: [])) ?: '—' ?></td></tr>
                <tr><td class="text-muted">Scanned</td><td><?= $site['scanned_at'] ? format_date($site['scanned_at']) : 'Never' ?></td></tr>
            </table>
        </div>
        ...
        <div class="card">
            ...
            <table>
                <tr><td class="text-muted" style="width:120px">Agent Mode</td><td><span class="badge badge-<?= $site['agent_mode'] === 'auto' ? 'approved' : 'draft' ?>"><?= $site['agent_mode'] ?></span></td></tr>
                <tr><td class="text-muted">Blog Path</td><td><?= e($site['blog_path']) ?></td></tr>
                <tr><td class="text-muted">Status</td><td><span class="badge badge-<?= $site['is_active'] ? 'approved' : 'rejected' ?>"><?= $site['is_active'] ? 'Active' : 'Inactive' ?></span></td></tr>
                <tr><td class="text-muted">Added</td><td><?= format_date($site['created_at']) ?></td></tr>
            </table>
        </div>
    </div>
    ...
    <!-- Agent Actions -->
    <div class="card">
        <div class="card-header">🤖 AI Agent Actions</div>
        <div class="flex gap-2" style="flex-wrap: wrap;">
            <a href="<?= url('/dashboard/agent-run.php?agent=auto-fixer&site=' . $site['id']) ?>" class="btn btn-sm" style="background:#ef4444;color:#fff;text-decoration:none;">🤖 Auto-Fix All Issues</a>
            <a href="<?= url('/dashboard/write.php?site=' . $site['id'] . '&step=propose') ?>" class="btn btn-accent btn-sm" style="text-decoration:none;">🧠 AI Content Planner</a>
            <a href="<?= url('/dashboard/agent-run.php?agent=scanner&site=' . $site['id']) ?>" class="btn btn-primary btn-sm" style="text-decoration:none;">🔍 Scan Site</a>
            <a href="<?= url('/dashboard/agent-run.php?agent=seo-auditor&site=' . $site['id']) ?>" class="btn btn-primary btn-sm" style="text-decoration:none;">📊 SEO Audit</a>
            <a href="<?= url('/dashboard/agent-run.php?agent=keyword-research&site=' . $site['id']) ?>" class="btn btn-primary btn-sm" style="text-decoration:none;">🔑 Find Keywords</a>
            <a href="<?= url('/dashboard/agent-run.php?agent=news-scraper&site=' . $site['id']) ?>" class="btn btn-primary btn-sm" style="text-decoration:none;">📰 Scrape News</a>
            <a href="<?= url('/dashboard/agent-run.php?agent=evaluator&site=' . $site['id']) ?>" class="btn btn-outline btn-sm" style="text-decoration:none;">📈 Evaluate Strategy</a>
        </div>
    </div>
