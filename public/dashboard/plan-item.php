...
<div class="pi-grid">
    ...
    <div>
        ...
        <?php else: ?>
            ...
            <?php if (!empty($faqs)): ?>
            <!-- Always-visible FAQ section: "what this post answers" -->
            <div style="margin-top:18px; padding:16px 18px; background:#fff; border:1px solid var(--border); border-radius:8px;">
                <div style="display:flex; justify-content:space-between; align-items:baseline; margin-bottom:6px;">
                    <h3 style="margin:0; font-size:14px; color:var(--primary);">Questions this post will answer</h3>
                    <span style="font-size:11px; color:var(--text-light);"><?= count($faqs) ?> Q&amp;A pairs</span>
                </div>
                <div style="font-size:11px; color:var(--text-light); margin-bottom:12px; line-height:1.5;">
                    Embedded inline at the end of the blog body AND emitted as <code>FAQPage</code> schema so Google can render expandable Q&amp;A in search results + AI engines can cite individual answers.
                </div>
                <div class="pi-faq-list">
                    <?php foreach ($faqs as $i => $qa): ?>
                        <details class="pi-faq" <?= $i < 2 ? 'open' : '' ?>>
